...
class Collection(DualIdMixin, Base):
    ...
    graph_state = Column(
        String(20), default="uninitialized", nullable=False
    )  # uninitialized/initializing/synchronized/out_of_sync/updating/error
    entities_hash = Column(
        Text, nullable=True
    )  # MD5 hash of source entities for change detection
    last_sync_timestamp = Column(
        DateTime(timezone=True), nullable=True
    )  # When graph was last synchronized
    ...
    collection_entities = relationship(
        "CollectionEntity",
        back_populates="collection",
        lazy="dynamic",
        cascade="all, delete-orphan",
    )
    collection_relationships = relationship(
        "CollectionRelationship",
        back_populates="collection",
        lazy="dynamic",
        cascade="all, delete-orphan",
    )
    entity_operations = relationship(
        "EntityOperation",
        back_populates="collection",
        lazy="dynamic",
        cascade="all, delete-orphan",
    )
    document_contexts = relationship(
    ...
    graph_entities = relationship(
        "GraphEntity", back_populates="source_collection", lazy="dynamic"
    )
    graph_relationships = relationship(
        "GraphRelationship", back_populates="source_collection", lazy="dynamic"
    )
    ...
    parent = relationship(
        "Collection", remote_side="Collection.uuid", back_populates="subcollections"
    )
    subcollections = relationship(
        "Collection", back_populates="parent", lazy="dynamic", cascade="all"
    )
    ...
    __table_args__ = (
        ...
        Index(
            "idx_collections_default_visibility_profile_uuid",
            "default_visibility_profile_uuid",
        ),
    )
...
class CollectionEntity(DualIdMixin, Base):
    ...
    display_name = Column(
        Text
    )  # Alternative display name (falls back to name if not set)
    is_merged = Column(
        Boolean, default=False, nullable=False
    )  # True if entity is result of merge operation
    created_from_link_uuid = Column(
        UUID(as_uuid=True),
        ForeignKey(
            "entity_links.uuid",
            ondelete="SET NULL",
            use_alter=True,
            name="fk_collection_entity_created_from_link",
        ),
        nullable=True,
    )  # EntityLink that created this merge
    lifecycle_state = Column(
        String(20), default="individual", nullable=False
    )  # individual/linked/merging/merged/unmerging/error
    operation_lock = Column(
        UUID(as_uuid=True), nullable=True
    )  # UUID of operation currently modifying this entity (for concurrency control)
    ...
    entity_links = relationship(
        "EntityLink",
        foreign_keys="[EntityLink.collection_entity_uuid]",
        back_populates="collection_entity",
        lazy="dynamic",
        cascade="all, delete-orphan",
    )
    source_entities = relationship(
        "GraphEntity",
    ...
    __table_args__ = (
        CheckConstraint(
            ...
            name="collection_entities_entity_id_not_empty",
        ),
        CheckConstraint(
            "length(trim(name)) > 0", name="collection_entities_name_not_empty"
        ),
    ...
    def to_dict(self):
        ...
        return {
            ...
            "created_from_link_uuid": (
                str(self.created_from_link_uuid)
                if self.created_from_link_uuid
                else None
            ),
...
class CollectionRelationship(DualIdMixin, Base):
    ...
    __table_args__ = (
        CheckConstraint(
            "length(trim(source_entity_id)) > 0",
            name="collection_relationships_source_not_empty",
        ),
        CheckConstraint(
            "length(trim(target_entity_id)) > 0",
            name="collection_relationships_target_not_empty",
        ),
...
class CollectionEntitySource(Base):
    ...
    collection_entity_uuid = Column(
        UUID(as_uuid=True),
        ForeignKey("collection_entities.uuid", ondelete="CASCADE"),
        primary_key=True,
    )
    source_graph_entity_uuid = Column(
        UUID(as_uuid=True),
        ForeignKey("graph_entities.uuid", ondelete="CASCADE"),
        primary_key=True,
    )
    created_at = Column(
        DateTime(timezone=True), nullable=False, server_default=func.now()
    ...
    __table_args__ = (
        Index(
            "idx_collection_entity_sources_collection_entity", "collection_entity_uuid"
        ),
        Index(
            "idx_collection_entity_sources_source_entity", "source_graph_entity_uuid"
        ),
    )
...
class CollectionRelationshipSource(Base):
    ...
    collection_relationship_uuid = Column(
        UUID(as_uuid=True),
        ForeignKey("collection_relationships.uuid", ondelete="CASCADE"),
        primary_key=True,
    )
    source_graph_relationship_uuid = Column(
        UUID(as_uuid=True),
        ForeignKey("graph_relationships.uuid", ondelete="CASCADE"),
        primary_key=True,
    )
    created_at = Column(
        DateTime(timezone=True), nullable=False, server_default=func.now()
    ...
    __table_args__ = (
        Index(
            "idx_collection_relationship_sources_collection_rel",
            "collection_relationship_uuid",
        ),
        Index(
            "idx_collection_relationship_sources_source_rel",
            "source_graph_relationship_uuid",
        ),
    )
...
class EntityLink(DualIdMixin, Base):
    ...
    merged_entity_uuid = Column(
        UUID(as_uuid=True),
        ForeignKey("collection_entities.uuid", ondelete="SET NULL"),
        nullable=True,
    )  # Result of merge
    ...
    created_by_tool = Column(
        Text, nullable=True
    )  # tool used to create to mirror entity
    ...
    collection_entity = relationship(
        "CollectionEntity",
        foreign_keys=[collection_entity_uuid],
    ...
    def get_entity_ids(self):
        ...
        if not self.linked_entities:
            return []
        return [
            le.get("entity_id") for le in self.linked_entities if le.get("entity_id")
        ]
...
class EntityOperation(DualIdMixin, Base):
    ...
    entity_id = Column(
        Text, nullable=True
    )  # Nullable for collection-level operations (sync_graph, initialize_graph)
...
class DocumentCollectionContext(DualIdMixin, Base):
    ...
    visibility_profile = relationship(
        "VisibilityProfile", foreign_keys=[visibility_profile_uuid]
    )
    ...
    __table_args__ = (
        ...
        Index(
            "idx_document_collection_contexts_visibility_profile_uuid",
            "visibility_profile_uuid",
        ),
    )
...
class VisibilityProfile(DualIdMixin, Base):
    ...
    file_uuid = Column(
        ...
        ForeignKey(
            ...
            ondelete="CASCADE",
            use_alter=True,
            name="fk_visibility_profile_document",
        ),
        nullable=True,
    )
    collection_uuid = Column(
        ...
        ForeignKey(
            ...
            ondelete="CASCADE",
            use_alter=True,
            name="fk_visibility_profile_collection",
        ),
        nullable=True,
    )
    ...
    auto_include_new = Column(
        Boolean, nullable=False, default=True
    )  # Auto-include new entities
    ...
    __table_args__ = (
        CheckConstraint(
            "length(trim(name)) > 0", name="visibility_profiles_name_not_empty"
        ),
        CheckConstraint(
    ...
    def __repr__(self):
        return (
            f"<VisibilityProfile(uuid={self.uuid}, id='{self.id}', name='{self.name}')>"
        )
