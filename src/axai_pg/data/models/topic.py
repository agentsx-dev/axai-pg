...
class Topic(DualIdMixin, Base):
    ...
    parent_topic_uuid = Column(
        UUID(as_uuid=True), ForeignKey("topics.uuid", ondelete="SET NULL")
    )
    ...
    __table_args__ = (
        CheckConstraint(
            "global_importance IS NULL OR (global_importance >= 0 AND global_importance <= 1)",
            name="topics_valid_importance",
        ),
        CheckConstraint("parent_topic_uuid != uuid", name="topics_no_self_reference"),
    )
...
class DocumentTopic(DualIdMixin, Base):
    ...
    topic_uuid = Column(
        UUID(as_uuid=True),
        ForeignKey("topics.uuid", ondelete="CASCADE"),
        nullable=False,
    )
