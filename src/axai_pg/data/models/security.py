...
class UserRole(DualIdMixin, Base):
    ...
    role_uuid = Column(
        UUID(as_uuid=True), ForeignKey("roles.uuid", ondelete="CASCADE"), nullable=False
    )
...
class RolePermission(DualIdMixin, Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "role_name", "resource_name", "permission_type", name="uq_role_permission"
        ),
    )
...
class AuditLog(DualIdMixin, Base):
    ...
    user_uuid = Column(
        UUID(as_uuid=True), ForeignKey("users.uuid", ondelete="SET NULL"), nullable=True
    )
    ...
    resource_uuid = Column(
        UUID(as_uuid=True)
    )  # Can reference records from different tables
...
class RateLimit(DualIdMixin, Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "user_uuid", "action_type", "window_start", name="uq_rate_limit"
        ),
    )
