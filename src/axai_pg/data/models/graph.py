...
class GraphEntity(DualIdMixin, Base):
    ...
    document = relationship(
        "Document", back_populates="graph_entity", foreign_keys=[document_uuid]
    )
    source_file = relationship(
        "Document", back_populates="graph_entities", foreign_keys=[source_file_uuid]
    )
    source_collection = relationship("Collection", back_populates="graph_entities")
    entity_links = relationship(
        "EntityLink",
        back_populates="graph_entity",
        lazy="dynamic",
    ...
    collection_entities_using = relationship(
        "CollectionEntity",
...
class GraphRelationship(DualIdMixin, Base):
    ...
    source_entity = relationship(
        "GraphEntity",
        foreign_keys=[source_entity_uuid],
        back_populates="outgoing_relationships",
    )
    target_entity = relationship(
        "GraphEntity",
        foreign_keys=[target_entity_uuid],
        back_populates="incoming_relationships",
    )
    document = relationship(
        "Document",
        back_populates="graph_relationships_rel",
        foreign_keys=[document_uuid],
    )
    source_file = relationship(
        "Document",
        foreign_keys=[source_file_uuid],
        overlaps="graph_relationships_rel,document",
    )
    source_collection = relationship("Collection", back_populates="graph_relationships")
    collection_relationships_using = relationship(
    ...
    __table_args__ = (
        ...
        CheckConstraint(
            "weight IS NULL OR weight > 0", name="graph_relationships_valid_weight"
        ),
        ...
        Index(
            "idx_graph_relationships_source_collection_uuid", "source_collection_uuid"
        ),
